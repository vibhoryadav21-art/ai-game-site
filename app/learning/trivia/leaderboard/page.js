...
export default function TriviaLeaderboardPage() {
  ...
  const loadCrewData = useCallback(async (userId) => {
    ...
    const [{ data: allCrews }, { data: myMemberships }, { data: myStats }] = await Promise.all([
      ...
      supabase.from('trivia_stats').select('default_crew_id').eq('user_id', userId).single(),
    ])
  ...
  async function createCrew() {
    ...
    await supabase.from('trivia_stats').update({ default_crew_id: crew.id }).eq('user_id', user.id)
