...
export default function SpanishLearnPage() {
  ...
  useEffect(() => {
    ...
    async function loadOrCreateStats() {
      ...
      if (error) {
        console.error('Failed to load spanish_stats:', error.message)
        setLoadingStats(false)
        return
      }
      ...
      if (data) {
        ...
        if (insertError) {
          console.error('Failed to create spanish_stats row:', insertError.message)
        } else {
          setStats(created)
        }
      }
  ...
  if (!user) {
    return (
      <div className="flex-1 bg-black text-zinc-100 flex flex-col items-center justify-center gap-4 p-6 text-center">
        <p className="text-zinc-300">Create a free account to track your Spanish progress.</p>
        <div className="flex gap-3">
          <Link
            href="/login?redirect=/learning/spanish"
            className="px-5 py-2 rounded-lg border border-zinc-700 text-zinc-200 hover:border-sky-400 transition"
          >
            Log in
          </Link>
          <Link
            href="/signup?redirect=/learning/spanish"
