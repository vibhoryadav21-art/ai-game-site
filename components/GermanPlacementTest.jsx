...
export default function GermanPlacementTest({ user, onComplete }) {
  ...
  async function finishTest(finalAnswers) {
    ...
    setResult({ determinedLevel, levelStats, totalCorrect });
    setPhase("result");
  }
  ...
  if (phase === "result") {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center gap-6 p-6">
        ...
        <p className="font-serif text-5xl text-amber-300">{result.determinedLevel}</p>
        <p className="text-zinc-400">
          {result.totalCorrect}/10 correct overall
        </p>
        ...
        <button
          onClick={() => onComplete(result.determinedLevel)}
          className="mt-4 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-medium px-6 py-3 rounded-xl transition"
        >
          Continue
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center gap-8 p-6">
